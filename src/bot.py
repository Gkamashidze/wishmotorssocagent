...
from src.database import get_last_category, set_last_category, save_post, mark_published, mark_skipped, get_last_pending_post, get_next_topic
from src.content import next_category, build_text_prompt, build_image_prompt, extract_parts_from_text, clean_text, CONTACT_INFO, TOPIC_LISTS
...
async def _generate_post(config: Config, category: str) -> dict[str, Any]:
    ...
    """
    specific_topic = await asyncio.to_thread(get_next_topic, category, TOPIC_LISTS[category])
    raw_text = await asyncio.to_thread(
        generate_post_text, build_text_prompt(category, specific_topic), config.gemini_api_key
    )
